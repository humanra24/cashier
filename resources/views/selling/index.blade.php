...
@section('content')
    ...
    <div class="row mb-3">
        <form action="{{ route('selling-temporary.store') }}" method="post" class="col-md-6">
            @csrf
            <div class="row">
                                    ...
                                    <tr>
                                        ...
                                        <td class="d-flex justify-content-center">
                                            <a href="#" class="me-3 text-warning"
                                                onclick="edit({
                                                url:'{{ route('selling-temporary.update', ['selling_temporary_detail' => $item->id]) }}',
                                                name:'{{ $item->name }}',
                                                qty:'{{ $item->qty }}'
                                            })"><i
                                                    class="fa-solid fa-pen-to-square"></i></a>
                                            <form class="deleteForm"
                                                action="{{ route('selling-temporary.destroy', ['selling_temporary_detail' => $item->id]) }}"
                                                method="post">
                                                @method('delete')
                                                @csrf
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control price @error('bayar') is-invalid @enderror"
                            ...
                            <div class="col-sm-10">
                                <div class="input-group">
                                    ...
                                    <input type="text" value="0" readonly
                                        class="form-control price @error('kembalian') is-invalid @enderror" id="kembalian"
                                        name="kembalian">
                                </div>
                                @error('kembalian')
                                    <div class="form-text text-danger">{{ $message }}</div>
                            ...
                            <tbody>
                                @foreach ($data['product'] as $item)
                                    <tr>
                                        ...
                                        <td class="d-flex justify-content-center">
                                            <form action="{{ route('selling-temporary.store') }}" method="post">
                                                @csrf
...
@push('js')
    @if (session('print'))
        <script>
            window.open('/selling-print', '_blank')
        </script>
    @endif
    <script>
        function edit(data) {
